...
char* calc::reverseString(const char* string,char length){
...
float calc::solve(const char* eq,char start,char end,const float* vars){
    ...
    std::vector<char> plusIndex;
    std::vector<char> multIndex;
    std::vector<char> powIndex;
    ...
    for (char i = end-1; i >= start; --i) {
        ...
        else if(eq[i] == '+')
        {
            if(tmpc>0)
            {
                ...
                if(!reversed)
                    return NAN;
                numbers.push_back(strtod(reversed,nullptr));
                plusIndex.push_back(numbers.size());
                free(reversed);
                tmpc=0;
            }
        ...
        //check for multiplication
            ...
            if(tmpc>0)
            {
                ...
                if(!reversed)
                    return NAN;
                numbers.push_back(strtod(reversed,nullptr));
                multIndex.push_back(numbers.size());
                free(reversed);
                tmpc=0;
            ...
            if(tmpc>0)
            {
                ...
                if(!reversed)
                    return NAN;
                numbers.push_back((float)1/strtod(reversed,nullptr));
                multIndex.push_back(numbers.size());
                free(reversed);
                tmpc=0;
            }else if(i==end-1 || i==start)
            ...
            if(tmpc>0)
            {
                ...
                numbers.push_back(strtod(reversed,nullptr));
                multIndex.push_back(numbers.size());
                powIndex.push_back(numbers.size());
                free(reversed);
                tmpc=0;
            }else if(i==end-1 || i==start)
            {
        ...
        //if we find a closing bracket, try to find a matching opening bracket and call solve recursively
            ...
            for(char j=i-1;j>=start;--j){
                ...
                else if(eq[j]=='(' && numClosingBrackets==0)
                {
                    //matching '(' found
                    if(!foundMatching) {
                        numbers.push_back(solve(eq, j + 1, i,vars));
